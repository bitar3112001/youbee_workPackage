...
function cross(...values) {
  ...
  while (true) {
    ...
    while (++index4[i] === lengths[i]) {
      if (i === 0) return reduce3 ? product.map(reduce3) : product;
      index4[i--] = 0;
    }
  }
}
...
function variance(values, valueof) {
  ...
  if (count4 > 1) return sum5 / (count4 - 1);
}
...
function fcumsum(values, valueof) {
  const adder = new Adder();
  let index4 = -1;
  return Float64Array.from(values, valueof === void 0 ? (v2) => adder.add(+v2 || 0) : (v2) => adder.add(+valueof(v2, ++index4, values) || 0));
}
...
function nest(values, map6, reduce3, keys) {
  return function regroup(values2, i) {
    if (i >= keys.length) return reduce3(values2);
...
function quantileIndex(values, p, valueof = number) {
  ...
  if (p <= 0) return minIndex(numbers3);
  if (p >= 1) return maxIndex(numbers3);
...
function pairs(values, pairof = pair) {
  ...
  let first = false;
  for (const value of values) {
    if (first) pairs3.push(pairof(previous, value));
    previous = value;
    first = true;
  }
  return pairs3;
...
function brush(dim) {
  var extent2 = defaultExtent, filter4 = defaultFilter2, touchable = defaultTouchable2, keys = true, listeners = dispatch_default("start", "brush", "end"), handleSize = 6, touchending;
...
function orient2dadapt(ax, ay, bx, by, cx, cy, detsum) {
  ...
  let bvirt, c6, ahi, alo, bhi, blo, _i, _j, _0, s1, s0, t14, t04, u32;
  const acx = ax - cx;
  const bcx = bx - cx;
  const acy = ay - cy;
  const bcy = by - cy;
...
function formatDate(date2) {
  var hours2 = date2.getUTCHours(), minutes2 = date2.getUTCMinutes(), seconds3 = date2.getUTCSeconds(), milliseconds3 = date2.getUTCMilliseconds();
  return isNaN(date2) ? "Invalid Date" : formatYear(date2.getUTCFullYear(), 4) + "-" + pad(date2.getUTCMonth() + 1, 2) + "-" + pad(date2.getUTCDate(), 2) + (milliseconds3 ? "T" + pad(hours2, 2) + ":" + pad(minutes2, 2) + ":" + pad(seconds3, 2) + "." + pad(milliseconds3, 3) + "Z" : seconds3 ? "T" + pad(hours2, 2) + ":" + pad(minutes2, 2) + ":" + pad(seconds3, 2) + "Z" : minutes2 || hours2 ? "T" + pad(hours2, 2) + ":" + pad(minutes2, 2) + "Z" : "");
}
...
function link_default(links) {
  var id2 = index2, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count4, bias, random, iterations2 = 1;
  if (links == null) links = [];
...
function circleStream(stream, radius, delta, direction, t04, t14) {
  ...
  if (t04 == null) {
    ...
    if (direction > 0 ? t04 < t14 : t04 > t14) t04 += direction * tau5;
  }
...
function line_default(a4, b, x06, y06, x12, y12) {
  var ax = a4[0], ay = a4[1], bx = b[0], by = b[1], t04 = 0, t14 = 1, dx = bx - ax, dy = by - ay, r;
  r = x06 - ax;
  if (!dx && r > 0) return;
  r /= dx;
  if (dx < 0) {
  ...
  if (t14 < 1) b[0] = ax + t14 * dx, b[1] = ay + t14 * dy;
  return true;
}
...
function computeUpperHullIndexes(points) {
  ...
  let size = 2, i;
  for (i = 2; i < n; ++i) {
    while (size > 1 && cross_default(points[indexes3[size - 2]], points[indexes3[size - 1]], points[i]) <= 0) --size;
    indexes3[size++] = i;
  }
...
function ticker(year2, month2, week, day2, hour2, minute2) {
  ...
  function tickInterval(start2, stop, count4) {
    ...
    if (i === tickIntervals.length) return year2.every(tickStep(start2 / durationYear, stop / durationYear, count4));
...
function bisector_default(f) {
  ...
  function left2(a4, x4, lo, hi) {
    ...
    while (lo < hi) {
      const mid = lo + hi >>> 1;
      if (compare(a4[mid], x4) < 0) lo = mid + 1;
      else hi = mid;
    }
    return lo;
  }
...
function ticker2(year2, month2, week, day2, hour2, minute2) {
  ...
  function tickInterval(start2, stop, count4) {
    ...
    if (i === tickIntervals.length) return year2.every(tickStep2(start2 / durationYear2, stop / durationYear2, count4));
    if (i === 0) return millisecond_default.every(Math.max(tickStep2(start2, stop, count4), 1));
    const [t, step] = tickIntervals[target / tickIntervals[i - 1][2] < tickIntervals[i][2] / target ? i - 1 : i];
    return t.every(step);
  }
  return [ticks2, tickInterval];
    ...
    return function(date2) {
      ...
      while (++i < n) {
        if (specifier.charCodeAt(i) === 37) {
          ...
          if (format2 = formats2[c6]) c6 = format2(date2, pad4);
          string.push(c6);
          j = i + 1;
        }
      }
...
function calendar(ticks2, tickInterval, year2, month2, week, day2, hour2, minute2, second3, format2) {
  ...
  function tickFormat2(date2) {
    return (second3(date2) < date2 ? formatMillisecond : minute2(date2) < date2 ? formatSecond : hour2(date2) < date2 ? formatMinute : day2(date2) < date2 ? formatHour : month2(date2) < date2 ? week(date2) < date2 ? formatDay : formatWeek : year2(date2) < date2 ? formatMonth : formatYear4)(date2);
  }
  scale2.invert = function(y4) {
...
function transformer3() {
  var x06 = 0, x12 = 1, t04, t14, k10, transform2, interpolator = identity3, clamp = false, unknown;
  function scale2(x4) {
...
function transformer4() {
  var x06 = 0, x12 = 0.5, x22 = 1, s2 = 1, t04, t14, t22, k10, k21, interpolator = identity3, transform2, clamp = false, unknown;
  function scale2(x4) {
...
function cornerTangents(x06, y06, x12, y12, r1, rc, cw) {
  var x01 = x06 - x12, y01 = y06 - y12, lo = (cw ? rc : -rc) / sqrt3(x01 * x01 + y01 * y01), ox = lo * y01, oy = -lo * x01, x11 = x06 + ox, y11 = y06 + oy, x10 = x12 + ox, y10 = y12 + oy, x004 = (x11 + x10) / 2, y004 = (y11 + y10) / 2, dx = x10 - x11, dy = y10 - y11, d2 = dx * dx + dy * dy, r = r1 - rc, D3 = x11 * y10 - x10 * y11, d = (dy < 0 ? -1 : 1) * sqrt3(max5(0, r * r * d2 - D3 * D3)), cx0 = (D3 * dy - dx * d) / d2, cy0 = (-D3 * dx - dy * d) / d2, cx1 = (D3 * dy + dx * d) / d2, cy1 = (-D3 * dx + dy * d) / d2, dx0 = cx0 - x004, dy0 = cy0 - y004, dx1 = cx1 - x004, dy1 = cy1 - y004;
  ...
  function arc() {
    ...
    if (!(r1 > epsilon8)) context.moveTo(0, 0);
      ...
      if (!(da1 > epsilon8)) context.moveTo(x01, y01);
        ...
        if (rc1 < rc) context.arc(t04.cx, t04.cy, rc1, atan22(t04.y01, t04.x01), atan22(t14.y01, t14.x01), !cw);
        else {
      ...
      if (!(r0 > epsilon8) || !(da0 > epsilon8)) context.lineTo(x10, y10);
        ...
        if (rc0 < rc) context.arc(t04.cx, t04.cy, rc0, atan22(t04.y01, t04.x01), atan22(t14.y01, t14.x01), !cw);
        else {
...
function zoom_default2() {
  ...
  zoom.scaleTo = function(selection2, k2, p, event) {
    zoom.transform(selection2, function() {
      var e = extent2.apply(this, arguments), t04 = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p, p1 = t04.invert(p02), k1 = typeof k2 === "function" ? k2.apply(this, arguments) : k2;
  ...
  function dblclicked(event, ...args) {
    if (!filter4.apply(this, arguments)) return;
    var t04 = this.__zoom, p02 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t04.invert(p02), k1 = t04.k * (event.shiftKey ? 0.5 : 2), t14 = constrain(translate(scale2(t04, k1), p02, p1), extent2.apply(this, args), translateExtent);
    noevent_default3(event);
    if (duration > 0) select_default2(this).transition().duration(duration).call(schedule, t14, p02, event);
    else select_default2(this).call(zoom.transform, t14, p02, event);
  }
  ...
  zoom.filter = function(_) {
    return arguments.length ? (filter4 = typeof _ === "function" ? _ : constant_default12(!!_), zoom) : filter4;
  };
  zoom.touchable = function(_) {
    return arguments.length ? (touchable = typeof _ === "function" ? _ : constant_default12(!!_), zoom) : touchable;
  };
  zoom.extent = function(_) {
    return arguments.length ? (extent2 = typeof _ === "function" ? _ : constant_default12([[+_[0][0], +_[0][1]], [+_[1][0], +_[1][1]]]), zoom) : extent2;
  };
...
export {
  ...
  defaultLocale3 as timeFormatDefaultLocale,
